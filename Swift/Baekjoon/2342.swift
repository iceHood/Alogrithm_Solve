...
let 명령들 = readLine()!.split(separator: " ").map({ Int($0)! })
let 비용패턴 = [
    [1, 2, 2, 2, 2], //0번에 있을 때
    [0, 1, 3, 4, 3], //1번에 있을 때
    [0, 3, 1, 3, 4], //2번에 있을 때
...
var 메모 = [[[Int]]](repeating: [[Int]](repeating: [Int](repeating: -1, count: 5), count: 5), count: 100001)
func 풀이(명령: Int, 왼발: Int, 오른발: Int) -> Int {
    if 명령 == 명령들.count - 1 { return 0 }
    if 왼발 == 오른발 && 왼발 != 0 { return 500000 }

    if 메모[명령][왼발][오른발] != -1 { return 메모[명령][왼발][오른발] }
    else {
        메모[명령][왼발][오른발] = min(풀이(명령: 명령 + 1, 왼발: 명령들[명령], 오른발: 오른발) + 비용패턴[왼발][명령들[명령]], 풀이(명령: 명령 + 1, 왼발: 왼발, 오른발: 명령들[명령]) + 비용패턴[오른발][명령들[명령]])
        return 메모[명령][왼발][오른발]
    }
}
print(풀이(명령: 0, 왼발: 0, 오른발: 0))

/*
 오른발 0 -> 1 위치: (0, 1) 총비용: 2
 왼발 0 -> 2 위치: (2, 1) 총비용: 4
 왼발 2 -> 3 위치: (3, 1) 총비용: 7
 오른발 1 -> 4 위치: (3, 4) 총비용: 10
 오른발 4 -> 4 위치: (3, 4) 총비용: 11
 왼발 3 -> 3 위치: (3, 4) 총비용: 12
 왼발 3 -> 2 위치: (2, 4) 총비용: 15
 오른발 4 -> 1 위치: (2, 1) 총비용: 18
 
 왼발 0 -> 1 위치: (1, 0) 총비용: 2
 오른발 0 -> 2 위치: (1, 2) 총비용: 4
 오른발 2 -> 3 위치: (1, 3) 총비용: 7
 왼발 1 -> 4 위치: (4, 3) 총비용: 10
 왼발 4 -> 4 위치: (4, 3) 총비용: 11
 오른발 3 -> 3 위치: (4, 3) 총비용: 12
 오른발 3 -> 2 위치: (4, 2) 총비용: 15
 왼발 4 -> 1 위치: (1, 2) 총비용: 18
 */
